...
{
	global $scripturl, $context, $settings, $txt, $modSettings, $user_profile;
	...
	$what = ssi_recentTopics($num_recent = $context['TPortal']['recentboxnum'] , $exclude_boards = array($bb),  $output_method = 'array');
		...
		foreach($what as $wi => $w) {
			...
			// Load posters profile
			loadMemberContext($w['poster']['id']);
			loadMemberData($w['poster']['id'], false, 'profile');

			// Find first and last post
			if($wi == 0)
				$queue = 'first';
			else if((count($what) -1) == $wi)
				$queue = 'last';
			else
				$queue = '';
			...
			// Add class to Poster
			foreach($user_profile as $profile) {
				if($profile['real_name'] == $w['poster']['name'])
	...
	else
	{
		...
		$coun = 1;
		echo '
		<ul class="recent_topics" style="' , isset($context['TPortal']['recentboxscroll']) && $context['TPortal']['recentboxscroll']==1 ? 'height: 20ex;' : '' , 'margin: 0; padding: 0;">';
		foreach($what as $wi => $w)
		{
			echo '
...
{
	global $context, $settings, $scripturl, $txt, $modSettings, $user_profile;
	...
	if(isset($context['TPortal']['userbox']['online']))
	{
		// add online users
		echo '
		<div class="block__inner block__users-online">';
		...
		foreach($online['users'] as $user) {
			...
			loadMemberData($user['id'], false, 'profile');

			foreach($user_profile as $profile) {

				if($user['name'] == $profile['real_name'])
					$class = $profile['options']['cust_class'];
			}
			...
			echo '<a href="'.$user['href'].'" class="class-color--'.$class.'">'.$user['name'].'</a>';
			echo '<br />';
		}
		echo '
			</div></div>';
		...
		loadMemberContext($modSettings['latestRealName']);
		loadMemberData($modSettings['latestRealName'], true, 'profile');

		foreach($user_profile as $user) {
			if($modSettings['latestRealName'] == $user['real_name']) 
					$class = $user['options']['cust_class'];
		}
		...
		echo '
		<h3 class="titlebg">'.$txt['members'].'</h3>
		<div class="block__inner members__stats">
		<div class="members">
		<p><span class="heavy">Newest: </span><a class="class-color--'.$class.'"href="', $scripturl, '?action=profile;u=', $modSettings['latestMember'], '"><strong>', $modSettings['latestRealName'], '</strong></a></p>
		<p><span class="heavy">'.$txt['total_members'].': </span>' , isset($modSettings['memberCount']) ? $modSettings['memberCount'] : $modSettings['totalMembers'] , '</p>
		<p><span class="heavy">'.$txt['total_posts'].': </span>'.$modSettings['totalMessages']. '</p>
		<p><span class="heavy">'.$txt['total_topics'].': </span>'.$modSettings['totalTopics']. '</p>
		<p><span class="heavy">'.$txt['tp-mostonline-today'].': </span>'.$modSettings['mostOnlineToday'].'</p>

		<h3>Currently Online</h3>

		<p><span class="heavy">Members: </span>'.$online['num_users']. '</p>
		<p><span class="heavy">'.$txt['tp-guests'].': </span>'.$online['guests'].'</p>
		<p><span class="heavy">'.$txt['tp-total'].': </span>'.$online['total_users'].'</p>
...
function TPortal_categorybox()
{
	...
	if(isset($context['TPortal']['blockarticle_titles'][$context['TPortal']['blocklisting']])){
		echo '<div class="middletext" ', (sizeof($context['TPortal']['blockarticle_titles'][$context['TPortal']['blocklisting']])>$context['TPortal']['blocklisting_height'] && $context['TPortal']['blocklisting_height']!='0') ? ' style="width: 100%; height: '.$context['TPortal']['blocklisting_height'].'em;"' : '' ,'>';
...
function TPblock($block, $theme, $side, $double=false)
{
	...
	<div class="', (($theme || $block['frame'] == 'frame') ? 'tborder tp_'.$side.'block_frame' : 'tp_'.$side.'block_noframe'), '">';
		...
		if (function_exists($func))
		{
			...
			echo '<div class="block" style="' , !empty($context['TPortal']['blockheight_'.$side]) ? 'height: '. ($double ? $nh : $context['TPortal']['blockheight_'.$side]) .';' : '' , '">';
			$func($block['id']);
			echo '</div>';
		}
		else
			echo '<div class="block" style="' , !empty($context['TPortal']['blockheight_'.$side]) ? 'height: '.$context['TPortal']['blockheight_'.$side].';' : '' , '">' , parse_bbc($block['body']) , '</div>';
...
function article_shortdate($render = true)
{
	...
	if(in_array('date',$context['TPortal']['article']['visual_options']))
		echo '
		<span class="article_shortdate right">' . tptimeformat($context['TPortal']['article']['date'], true, '%d %b %Y').'</span>';
	else
		echo '';
}
...
function article_category($render=true)
{
	...
	{
		//if(isset($context['TPortal']['article']['boardnews']))
			//echo '<span class="article_category">' . $txt['tp-fromcategory'] . '<a href="' . $scripturl . '?board=' . $catNameOrId . '">' . $context['TPortal']['article']['category_name'] . '</a></span>';
		//else
			//echo '<span class="article_category">' . $txt['tp-fromcategory'] . '<a href="' . $scripturl . '?cat=' . $catNameOrId . '">' . $context['TPortal']['article']['category_name'] . '</a></span>';
	}
}
