...
<div id="profileview" class="flow_auto">
	...
	echo '<div class="membergroup">';
	if (!empty($context['member']['group']))
		echo $context['member']['group'];
	else
		echo 'Applicant';
	echo '</div>';
	...
	echo '
		<div class="avatar">
			...
			if($context['member']['online']['label'] == 'Online')
				$icon = '/inde/classicon-'.strtolower($context['member']['options']['cust_class']).'.png';
			else {
				$icon = '/inde/classicon-'.strtolower($context['member']['options']['cust_class']).'-dis.png';
			}

			if(!$context['member']['avatar']['image']) {

				echo '
				<div class="member__image member__image--no-image round  class-background--'.str_replace(' ', '_', strtolower($context['member']['options']['cust_class'])).'">'.substr($context['member']['name'], 0, 2).'
					<div class="member__icon round class-border--'.str_replace(' ', '_', strtolower($context['member']['options']['cust_class'])).'" style="background-image: url('.$icon.')"></div>
				</div>';
			} else {

				$src = explode('src="', $context['member']['avatar']['image']);
				$src = explode('"', $src[1]);
				echo '
	...
	if ($context['member']['show_email'] === 'yes' || $context['member']['show_email'] === 'no_through_forum' || $context['member']['show_email'] === 'yes_permission_override')
		echo '
			<div class="member__profile__icon">
				<a
					href="', $scripturl, '?action=emailuser;sa=email;uid=', $context['member']['id'], '"
					title="', $context['member']['show_email'] == 'yes' || $context['member']['show_email'] == 'yes_permission_override' ? $context['member']['email'] : '', '"
					rel="nofollow"
					class="member__profile--icon round icon-mail-1">
				</a>
			</div>';

			echo '
				<div class="member__profile__icon">
					<a href="', $scripturl, '?action=profile;area=showposts;u=', $context['member']['id'], '" class="member__profile--icon round icon-doc-text-inv"></a>
				</div>';

			echo '
				<div class="member__profile__icon">
					<a href="', $scripturl, '?action=profile;area=statistics;u=', $context['member']['id'], '" class="member__profile--icon round icon-chart-bar-1"></a>
				</div>';
...
echo '</div';
	...
	// 			// echo '<li class="custom_field">', $field['output_html'], '</li>';
	// }
	//
	// echo '
